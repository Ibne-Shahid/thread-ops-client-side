...
import { 
  ...
  FaClock, FaEdit, FaTrash, FaCrown, FaSearch, FaSortUp, FaSortDown
} from 'react-icons/fa';
...
const UsersTable = ({ allUsers: users = [], title = "User Management", onUserAction }) => {
  ...
  const handleApprove = (userId) => {
    if (onUserAction) onUserAction('approve', userId);
  };

  const handleMakeAdmin = (userId) => {
    if (onUserAction) onUserAction('makeAdmin', userId);
  };

  const handleDelete = (userId) => {
    if (onUserAction) onUserAction('delete', userId);
  };

  const handleBulkAction = (action) => {
    if (onUserAction && selectedUsers.length > 0) {
      onUserAction(action, selectedUsers);
    }
  };
